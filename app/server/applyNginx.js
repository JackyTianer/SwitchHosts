...
const io = require('./io');
const { sys_nginx_path, work_path } = require('./paths');
...
function apply_Unix(content, callback) {
  ...
  io.pWriteFile(tmp_fn, content)
    .then(() => {
      ...
      if (!sudo_pswd) {
        cmd = [
          `cat "${tmp_fn}" > ${sys_nginx_path}`
          , `rm -rf ${tmp_fn}`
        ].join(' && ');

      } else {
        cmd = [
          `echo '${sudo_pswd}' | sudo -S chmod 777 ${sys_nginx_path}`
          , `cat "${tmp_fn}" > ${sys_nginx_path}`
          , `echo '${sudo_pswd}' | sudo -S chmod 644 ${sys_nginx_path}`
          // , 'rm -rf ' + tmp_fn
        ].join(' && ');
      }
  ...
  return Promise.resolve()
    ...
    .then(() => {
      return new Promise((resolve, reject) => {
        let file_md5 = md5File.sync(sys_nginx_path);
