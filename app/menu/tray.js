...
const getUserNginxConfigs = require('../server/actions/getUserNginxConfigs');
...
function makeMenu(app, list, nginx_list, contents, sys_lang) {
  let menu = [];
  ...
  nginx_list.map((item, idx) => {
    menu.push({
      ...
      accelerator: ac[idx],
      click: () => {
        svr.broadcast('toggle_nginx', Object.assign({}, item));
        //svr.emit('update_tray')
      }
    });
  });
...
function makeTray(app, contents, sys_lang = 'en') {
  ...
  svr.on('update_tray', () => {
    Promise.all([getUserHosts(), getUserNginxConfigs()])
      .then((result) => {
        const list = result[0];
        const nginx_list = result[1];
        let contextMenu = Menu.buildFromTemplate(makeMenu(app, list, nginx_list, contents, sys_lang));
